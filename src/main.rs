...
fn pre_commit(repo_url: &str) -> Result<(), Box<dyn std::error::Error>> {
    // Load the stores
...
/// List all configured repositories
fn repo_list() -> Result<(), Box<dyn std::error::Error>> {
    let repo_store = RepoStore::load()?;
...
/// Delete a repository configuration
fn repo_delete(name: Option<String>) -> Result<(), Box<dyn std::error::Error>> {
    let mut repo_store = RepoStore::load()?;
...
/// List all configured profiles
fn profile_list() -> Result<(), Box<dyn std::error::Error>> {
    let config_store = ConfigStore::load()?;
...
/// Delete a profile configuration (also removes related repositories)
fn profile_delete(name: Option<String>) -> Result<(), Box<dyn std::error::Error>> {
    let mut config_store = ConfigStore::load()?;
    let mut repo_store = RepoStore::load()?;
...
fn main() {
    ...
    match cli.command {
        ...
        Commands::Repo { action } => match action {
            RepoCommands::List => {
                if let Err(e) = repo_list() {
                    eprintln!("Error: {}", e);
                    std::process::exit(1);
                }
            }
            RepoCommands::Delete { name } => {
                if let Err(e) = repo_delete(name) {
                    eprintln!("Error: {}", e);
                    std::process::exit(1);
                }
            }
        ...
        Commands::Profile { action } => match action {
            ProfileCommands::List => {
                if let Err(e) = profile_list() {
                    eprintln!("Error: {}", e);
                    std::process::exit(1);
                }
            }
            ProfileCommands::Delete { name } => {
                if let Err(e) = profile_delete(name) {
                    eprintln!("Error: {}", e);
                    std::process::exit(1);
                }
            }
